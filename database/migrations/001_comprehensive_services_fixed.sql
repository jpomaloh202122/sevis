...
-- 2. Update users table to support enhanced admin roles (if exists)
DO $$ 
BEGIN
...
-- 3. Create comprehensive_services table
...
-- 4. Insert comprehensive services data
...
('Driver License', 'drivers_license', 'G2C', 'available', 'Online application for provisional and full driver licenses with document upload.', '4-6 weeks', 28, 300.00, 'high', ARRAY['citizens', 'residents']),
('Learner Permit', 'learners_permit', 'G2C', 'available', 'Apply for learner permits online with medical certificate verification.', '3-4 weeks', 21, 200.00, 'high', ARRAY['citizens', 'residents']),
...
-- 5. Create application_workflow table (if applications table exists)
DO $$ 
BEGIN
...
-- 6. Create application_documents table (if applications table exists)
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'applications') THEN
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'application_workflow') THEN
...
-- Create application summary view only if applications table exists
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'applications') 
       AND EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'users') THEN
        ...
        EXECUTE 'CREATE OR REPLACE VIEW v_application_summary AS
        SELECT 
          a.*,
          ...
          COALESCE(aw_count.workflow_steps_count, 0) as workflow_steps_count,
          COALESCE(ad_count.documents_count, 0) as documents_count,
          COALESCE(ad_verified.verified_documents_count, 0) as verified_documents_count
        FROM applications a
        JOIN users u ON a.user_id = u.id
        LEFT JOIN comprehensive_services cs ON cs.name = a.service_name
        LEFT JOIN (
          SELECT application_id, COUNT(*) as workflow_steps_count 
          FROM application_workflow 
          GROUP BY application_id
        ) aw_count ON aw_count.application_id = a.id
        LEFT JOIN (
          SELECT application_id, COUNT(*) as documents_count 
          FROM application_documents 
          GROUP BY application_id
        ) ad_count ON ad_count.application_id = a.id
        LEFT JOIN (
          SELECT application_id, COUNT(*) as verified_documents_count 
          FROM application_documents 
          WHERE is_verified = true 
          GROUP BY application_id
        ) ad_verified ON ad_verified.application_id = a.id';
    END IF;
END $$;

-- 9. Create triggers for automatic timestamping
...
-- Create application timestamp trigger if table exists
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'applications') THEN
        DROP TRIGGER IF EXISTS applications_update_timestamp ON applications;
        EXECUTE 'CREATE TRIGGER applications_update_timestamp
...
DROP POLICY IF EXISTS "Public read access to available services" ON comprehensive_services;
CREATE POLICY "Public read access to available services" ON comprehensive_services
  FOR SELECT TO public USING (service_status IN ('available', 'coming_soon'));

-- RLS policies for workflow and documents tables (if they exist)
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'application_workflow') THEN
        ALTER TABLE application_workflow ENABLE ROW LEVEL SECURITY;
        
        DROP POLICY IF EXISTS "Users can view their own application workflows" ON application_workflow;
        ...
        DROP POLICY IF EXISTS "Admins have full access to workflows" ON application_workflow;
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'application_documents') THEN
        ...
        DROP POLICY IF EXISTS "Admins have full access to documents" ON application_documents;
